                ...
                <div class="divide-y divide-gray-50 dark:divide-zinc-800/50">
                    @forelse($staff as $member)
                        <div class="px-7 py-5 flex items-center justify-between hover:bg-gray-50/50 dark:hover:bg-zinc-800/20 transition-colors group">
                            ...
                            <div class="flex items-center space-x-3">
                                <flux:badge size="sm" class="font-bold uppercase tracking-wider border-0 {{ $member->role === 'business_owner' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-[#14B8A6]' : 'bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-gray-400' }}">
                                    {{ str_replace('_', ' ', Str::title($member->role)) }}
                                </flux:badge>
                                @if($member->id !== auth()->id() && auth()->user()->role === 'business_owner')
